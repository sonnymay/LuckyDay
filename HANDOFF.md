...
#### Polish sprint — solar-term tooltip, brand sweep, copy unification
Six small commits in one continuous polish run after the pass-5 review fixes shipped. No new features; all are voice / token / craft work that compounds across every user session.

- **Solar-term explainer tooltip** (`app/detail.tsx`, 16e2374) — mirrors the double-hour pattern. "6 days until Grain Buds ⓘ" → tap → champagne bubble: "Solar terms are 24 seasonal markers in the Chinese almanac — small turning points in the year." Same 220ms fade-in, 4s auto-dismiss, tap-again to dismiss early. Cleanup timer paired with the double-hour one.
- **Full lavender brand sweep** (16e2374) — removed `colors.lavender` from all 6 remaining surfaces (history `statDivider`, settings `howItWorksCard`, home `moonStrip`+`guidanceCard`, privacy `summaryCard`, terms `summaryCard`, `LuckyMetricCard.timeCard`). All replaced with `colors.panelStrong` + `colors.roseGold`. Killed every hardcoded `'#C8BFEE'` border too. Brand token system is now consistent across every screen.
- **Score-band + delta nuance refined** (02a0560) — extended reviewer-praised pattern ("a quieter day, not a bad one"). `getScoreContext` bottom two tiers rewritten so every tier feels chosen, not punished. `getDeltaExplanation` expanded from 2 down-tiers to 3 (added mid-5-9 band "softer ground, choose one focus and protect it"), same for up. Same-day reframed to "Same tone as yesterday — your rhythm holds."
- **Paywall copy sharpened** (95a4407) — `FEATURES` list reordered + added auspicious-day insights bullet; subhead replaced generic "deeper readings" verb soup with concrete value list; compare grid Premium column leads with auspicious + ends with almanac-framed share cards; locked-preview headline tightened.
- **Settings + feedback voice unified** (3f07f53) — "Saved locally on this device" → "Stays on your device — never sent anywhere"; "How LuckyDay works" tightened with a clean disclaimer line; feedback "We predicted:" → "Almanac said:" (drops engineering-pronoun voice for the almanac-as-authority framing); feedback saved screen "✨ Reflection saved" → "✦ Noted" (unifies with the journal echo's "✦ Noted" pattern — two acknowledgment surfaces now share one ritual word).
- **Notification copy — horoscope drift removed** (5c60de5) — replaced 3 weak titles + 2 weak bodies in the daily push pool. Drift offenders: "What does today hold for you?" (fortune-cookie), "The stars aligned…" (astrology), "today's chart" (horoscope). All swapped for almanac-register equivalents. Compounds across every push the user receives.

#### Double-hour explainer tooltip — acquisition unlock
- **Tap-to-reveal explainer on the double-hour chip** (`app/detail.tsx`) — pass-5 reviewer flagged that "Pig hour · 9-11pm" is decorative noise for new users with no explanation. Chip is now a `Pressable` with an `ⓘ` glyph; tap opens a champagne-bordered tooltip below: "Chinese time-keeping splits the day into 12 two-hour periods, each named after a zodiac animal." 220ms fade-in, auto-dismiss after 4s (or tap-again to dismiss early). New `toggleDoubleHourTooltip()` handler manages timer cleanup on unmount. Educates without cluttering — old users skip it, new users learn the concept in one tap.

#### Pass-5 review fixes — journal echo, journal label sync, Settings copy, desktop max-width
- **Journal echo** (`app/detail.tsx`) — reviewer's pick for the $8.49 → $9.99 unlock: "the app heard you." On blur with non-empty text, a 280ms fade-in shows the user's first 6–8 words back in italic mauve below the field: `"<their words>" ✦ Noted` — holds 2.4s, then 480ms fade-out. New `triggerJournalEcho(text)` helper handles the truncation + animation; new `journalEcho` / `journalEchoAnim` state pair. No new storage — the echo is purely emotional acknowledgment, the entry is already persisted.
- **Journal label state sync** (`app/detail.tsx`) — pass-5 reviewer caught the bug: the outer label said "What's on your mind today?" while the inner placeholder shifted to "How did it go?" after ritual completion — two voices. Now the label itself swaps with the same trigger ("How did it go?" once `ritualDone`), and the placeholder rewords ("A line about how it went…") so they reinforce rather than contradict.
- **Settings upgrade copy** (`app/settings.tsx`) — reviewer flagged "You're on Free" as machine-translation feel and "Upgrade for full rituals, deeper history, and more." as weak (the "and more" especially). Now: title `"Free plan"` (canonical SaaS language), sub `"Premium unlocks full daily almanac, unlimited reading history, and the auspicious-day insights."` (concrete value, no "and more").
- **Desktop max-width guard** (`src/components/Screen.tsx`) — review found that at 1091px+ widths the score orb + cards stretch uncomfortably (this is a portrait phone surface, not a desktop app). Added web-only `maxWidth: 430` + `alignSelf: 'center'` + `width: '100%'` to the content container so desktop preview/embed renders as a centered phone column.

#### Pass-4 review fixes — chime envelope, button weight, web share, confetti burst
- **Ritual tap button text weight fixed** (`app/detail.tsx`) — reviewer found that despite `fontWeight: '900'` in the style, the rendered weight on web was 400. Cause: `fontFamily: fonts.heavy` (= `Nunito-Black`) wasn't loaded on web, and when the family is missing RN Web silently falls back to system font + ignores the weight. Dropped `fontFamily` from `ritualTapText` so `fontWeight: '900'` actually takes effect on both platforms.
- **Chime envelope rebuilt** (`src/lib/chime.ts`) — pass-3 sine pair clicked/popped because of a 20ms attack ramp. Now: 80ms soft linear attack from literal zero, 60ms peak hold, exponential decay to silence over the rest of the note. Added a third note (G5 for a full major chord), small ±4-cent detune per oscillator (chorus shimmer), and a 2200Hz lowpass biquad filter so the harmonics read as warm wood-bowl rather than test-tone. Per-note peaks scaled (0.16/0.12/0.08) to keep the chord balanced.
- **Web share refactored to eager bypass** (`app/detail.tsx`) — pass-3's error-catch was inconsistent on desktop Chrome (which has `navigator.share` but rejects on user-gesture grounds). Refactored: any `Platform.OS === 'web'` invocation now goes straight to the explainer alert. No `Share.share` call attempted off-iOS/Android.
- **Ritual micro-confetti** (`app/detail.tsx`) — reviewer's pick for next ≤1-day feature. 8 color-coded dots fan outward from the button center on tap, each at a fixed compass angle (0°/45°/90°/…/315°), with parabolic translateY (lifts past target then settles) and a 0.4 → 1 → 0.7 scale curve. Colors rotate gold / rose-gold / today's lucky color so the burst feels personal. Single `Animated.Value` drives all 8 dots via interpolation — no extra state. 700ms total, eased cubic-out. Pointer-events disabled on the overlay so dots never block re-tap. New `ConfettiBurst` sub-component, new `ritualTapWrap` container for positioning.
